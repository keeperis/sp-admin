...
const fetcher = async (url: string) => {
  ...
  if (!res.ok) {
    throw new Error(data?.error || 'Nepavyko įkelti turinio');
  }
  return data;
};
...
async function translateLtToEn(texts: string[]) {
  ...
  const data = await res.json();
  if (!res.ok) throw new Error(data?.error || 'Vertimas nepavyko');
...
export default function ContentPage() {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(body?.error || 'Išsaugoti nepavyko');
      }
  ...
  return (
    <Container size="lg">
      ...
      <Group gap="xs" mb="md" wrap="wrap">
        {[
          { label: 'Pradžia', value: 'hero' },
          { label: 'Vienkartiniai užsiėmimai', value: 'oneTime' },
          { label: 'Nuolatiniai užsiėmimai', value: 'ongoing' },
          { label: 'Privatūs užsiėmimai', value: 'private' },
          { label: 'DUK', value: 'faq' },
          { label: 'Apie mus', value: 'about' },
        ].map((item) => (
          <Button
            key={item.value}
            size="xs"
      ...
      <Accordion variant="separated" radius="md" value={activeSection}>
        {activeSection === 'hero' && (
          <Accordion.Item value="hero">
            <Accordion.Control>
              <Text fw={600}>Pradžia</Text>
            </Accordion.Control>
            <Accordion.Panel>
              <Stack gap="sm">
                <TextInput
                  label="Prekės ženklo pavadinimas (LT)"
                ...
                <TextInput
                  label="Prekės ženklo pavadinimas (EN)"
                ...
                <Group justify="space-between">
                  <Text fw={600}>Eilutės (daugiausia 10, po 3 žodžius)</Text>
                  <Button
                    size="xs"
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        try {
                          ...
                          const source = [
                            content.sections[sectionKey].title.lt,
                            ...withFiveParagraphs(content.sections[sectionKey].paragraphs).map(
                              (p) => p.lt,
                            ),
                          ];
                          ...
                          notifications.show({
                            message: error?.message || 'Vertimas nepavyko',
                            color: 'red',
                          });
                        } finally {
                          setTranslating(null);
            ...
            <Accordion.Panel>
              <Stack gap="sm">
                ...
                <Group justify="flex-end">
                  <Button
                    ...
                    onClick={async () => {
                      try {
                        ...
                        setDraft(next);
                        notifications.show({
                          message: 'Skiltis „Apie mus“ išversta',
                          color: 'green',
                        });
                      } catch (error: any) {
                        notifications.show({
                  ...
                  >
                    Versti visą skiltį „Apie mus“ LT → EN
                  </Button>
                </Group>
              </Stack>
            </Accordion.Panel>
